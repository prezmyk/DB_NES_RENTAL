...
CREATE SEQUENCE cartridges_id_seq START WITH 1000 INCREMENT BY 10 MINVALUE 1000 NOCACHE ORDER;

CREATE SEQUENCE cust_emp_seq START WITH 100 INCREMENT BY 1 MAXVALUE 999 MINVALUE 100 CYCLE NOCACHE;

CREATE SEQUENCE rentals_id_seq START WITH 1 INCREMENT BY 1 MINVALUE 1 CACHE 10 ORDER;

CREATE SEQUENCE reservations_id_seq START WITH 1 INCREMENT BY 1 MINVALUE 1 CACHE 10 ORDER;
...
ALTER TABLE addresses
    ADD CONSTRAINT addresses_phone_ck CHECK ( regexp_count(phone, '\D') = 0 );

ALTER TABLE addresses
    ADD CONSTRAINT addresses_email_ck CHECK ( regexp_count(email, '@') = 1 );
...
COMMENT ON COLUMN addresses.phone IS
    'Phone number. Only numbers 0-9.';

COMMENT ON COLUMN addresses.email IS
    'Address e-mail.';

ALTER TABLE addresses ADD CONSTRAINT addresses_pk PRIMARY KEY ( address_id );

CREATE TABLE cartridges (
    cartridge_id NUMBER DEFAULT cartridges_id_seq.NEXTVAL NOT NULL,
    title        VARCHAR2(100),
...
CREATE TABLE cartridges_rentals (
...
COMMENT ON COLUMN cartridges_rentals.cartridge_id IS
    'Multiple Primary Key with column rental_id of cartridges_rentals table.';

COMMENT ON COLUMN cartridges_rentals.rental_id IS
    'Multiple Primary Key with column cartridgel_id of cartridges_rentals table.';

ALTER TABLE cartridges_rentals ADD CONSTRAINT cartriges_rental_pk PRIMARY KEY ( cartridge_id,
                                                                                rental_id );
...
COMMENT ON COLUMN customers.patron_id IS
    'Patron ID of the customer who is under 18 years old.  Foreign key to customer_id column of customers table.';
...
CREATE TABLE rentals (
    rental_id   NUMBER DEFAULT rentals_id_seq.NEXTVAL NOT NULL,
...
CREATE TABLE reservations (
    reservation_id NUMBER DEFAULT reservations_id_seq.NEXTVAL NOT NULL,
...
COMMENT ON COLUMN reservations.customer_id IS
    'Customer ID for the reservation.  Foreign key to customer_id column in the customers table. Max 3 carts per customer.';

COMMENT ON COLUMN reservations.cartridge_id IS
    'Cartridge ID for the reservation..  Foreign key to cartridge_id column in the cartridges table. Max 3 carts per customer.';
...
ALTER TABLE cartridges_rentals
    ADD CONSTRAINT cartriges_rentals_cartriges_fk FOREIGN KEY ( cartridge_id )
        REFERENCES cartridges ( cartridge_id )
    NOT DEFERRABLE;

ALTER TABLE cartridges_rentals
    ADD CONSTRAINT cartriges_rentals_rentas_fk FOREIGN KEY ( rental_id )
